...
public class EnemyShipLevel2 : MonoBehaviour
{
    ...
    private IEnumerator MoveDownRoutine()
    {
        while (true)
        {
            yield return new WaitForSeconds(EnemyMovement.waitForSeconds2);  // Wait for 4 seconds
            transform.position += new Vector3(0, -EnemyMovement.moveDistance2, 0);  // Move the ship down
        }
    }
